...
from readio.utils.auth import check_tokens_get_state
...
def addPost():
    try:
        state = check_tokens_get_state(request.cookies.get('Admin-Token'), 'tagger')
        if state == 404:
...
def getPost():
    try:
        ...
        state = check_tokens_get_state(request.cookies.get('Admin-Token'), 'common')
        if state == 404:
            return build_error_response(400,'会话未建立，请重新登录')
        elif state == 403:
            return build_error_response(403,'您没有该操作的权限，请联系管理员')
