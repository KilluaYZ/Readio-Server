...
def register_user_sql(data):
    try:
        conn, cursor = pooldb.get_conn()
        cursor.execute('insert into user(username,nickname,password,roles) values(%s,%s,%s,%s)',
                       (data['username'], data['username'], generate_password_hash(data['password']), 'common'))
        conn.commit()
        pooldb.close_conn(conn, cursor)
